...
class TrustedPolygonRPCAdapter:
    ...
    def _provider_snapshot(self, provider_id: str, tx_hash: str) -> dict[str, Any]:
        ...
        return {"provider_id":provider_id,"chain_id":chain_id,"from":_address(tx.get("from"),"sender"),"to":_address(tx.get("to"),"recipient"),"value":_wei_to_pol(tx.get("value")),"receipt_status":_status(receipt.get("status")),"tx_hash":observed_tx,"block_hash":tx_block_hash,"tx_block_number":tx_block,"finalized_block_number":finalized_block}

    def build_trusted_observation(self, *, intent_id: str, tx_hash: str, provider_ids: Iterable[str]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        ...
        economic_keys=("chain_id","from","to","value","receipt_status","tx_hash","block_hash","tx_block_number")
        ...
        first=snapshots[0]
        if first["chain_id"]!=CHAIN_ID or first["chain_id"]!=intent["chain_id"]: raise EvidencePaymentError("RPC_CHAIN_MISMATCH","Wrong chain")
        if first["from"]!=str(intent["payer"]).lower(): raise EvidencePaymentError("RPC_SENDER_MISMATCH","Wrong sender")
        if first["to"]!=str(intent["treasury_address"]).lower(): raise EvidencePaymentError("RPC_TREASURY_MISMATCH","Wrong treasury")
        if first["value"]!=str(intent["expected_value"]): raise EvidencePaymentError("RPC_VALUE_MISMATCH","Wrong payment value")
        if first["receipt_status"]!=1: raise EvidencePaymentError("RPC_RECEIPT_FAILED","Transaction receipt is not successful")
        provider_observations=[{"provider_id":s["provider_id"],"tx_hash":s["tx_hash"],"block_hash":s["block_hash"],"receipt_status":s["receipt_status"],"tx_block_number":s["tx_block_number"],"finalized_block_number":s["finalized_block_number"],"chain_id":s["chain_id"],"from":s["from"],"to":s["to"],"value":s["value"],"asset":"POL"} for s in snapshots]
        observation={"chain_id":first["chain_id"],"from":first["from"],"to":first["to"],"value":first["value"],"asset":intent["asset"],"receipt_status":first["receipt_status"],"case_id":intent["case_id"],"entitlement_ref":intent["entitlement_ref"],"tx_hash":first["tx_hash"],"block_hash":first["block_hash"],"block_number":first["tx_block_number"],"rpc_provenance":{"version":RPC_PROVENANCE_VERSION,"provider_ids":sorted(ids),"methods":["eth_chainId","eth_getTransactionByHash","eth_getTransactionReceipt","eth_getBlockByNumber(finalized)"]}}
        return observation,provider_observations

    def _mark_manual_review(self,intent_id:str,reason:str)->dict[str,Any]:
        current=self.engine.get_intent(intent_id); state=current["state"]
        if state in {"INTENT_CREATED","USER_ACTION_REQUIRED"}:
            if state=="INTENT_CREATED": self.engine.transition_payment(intent_id,"USER_ACTION_REQUIRED","payment action observed")
            self.engine.transition_payment(intent_id,"TX_OBSERVED","transaction hash submitted to server verifier"); state="TX_OBSERVED"
        if "MANUAL_REVIEW" in PAYMENT_TRANSITIONS.get(state,set()): self.engine.transition_payment(intent_id,"MANUAL_REVIEW",reason)
    ...
    def settle_from_tx_hash(self, *, intent_id: str, tx_hash: str, provider_ids: Iterable[str]) -> dict[str, Any]:
        ...
        if current["state"]=="SETTLED":
            ...
            return {"intent_id":intent_id,"verdict":"SETTLED","entitlement_granted":True,"settlement_certificate_id":certificate["certificate_id"],"idempotent":True}
        if current["state"] in {"EXPIRED","REJECTED","MANUAL_REVIEW"}: return {"intent_id":intent_id,"verdict":"MANUAL_REVIEW","entitlement_granted":False}
        if current["state"]=="INTENT_CREATED": current=self.engine.transition_payment(intent_id,"USER_ACTION_REQUIRED","payment action observed")
        if current["state"]=="USER_ACTION_REQUIRED": current=self.engine.transition_payment(intent_id,"TX_OBSERVED","transaction hash submitted to server verifier")
        if current["state"]=="TX_OBSERVED": current=self.engine.transition_payment(intent_id,"VERIFYING","server-side RPC provenance verification")
        try: observation,providers=self.build_trusted_observation(intent_id=intent_id,tx_hash=tx_hash,provider_ids=provider_ids)
        except EvidencePaymentError as exc: return self._mark_manual_review(intent_id,exc.code)
        if current["state"]=="VERIFYING": self.engine.transition_payment(intent_id,"FINALITY_PENDING","trusted RPC evidence bound")
