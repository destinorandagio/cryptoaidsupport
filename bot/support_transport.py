...
class SupportTransportStore:
    ...
    def create_ticket(self, *, request: SupportRequest, telegram_principal: str) -> TicketReceipt:
        if not isinstance(request, SupportRequest):
            raise SupportTransportRejected("authorized_support_request_required")
        principal_digest = _digest(telegram_principal)
        fingerprint = self._ticket_fingerprint(request, principal_digest)
        created_at = _now()
        ticket_id = "st_" + uuid.uuid4().hex
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            existing = connection.execute(
                "SELECT * FROM support_tickets WHERE request_fingerprint=?", (fingerprint,)
            ).fetchone()
            if existing:
                ...
                return TicketReceipt(
                    ...
                    escalate=bool(existing["escalate"]),
    ...
    def _delivery_fingerprint(notification: SafeCaseNotification, principal_digest: str) -> str:
        canonical = f"{notification.idempotency_key}|{principal_digest}".encode("utf-8")
        return sha256(canonical).hexdigest()

    def claim_notification(
        self,
        *,
        notification: SafeCaseNotification,
        telegram_principal: str,
        lease_seconds: int = 60,
        now: float | None = None,
    ) -> NotificationClaim:
        if not isinstance(notification, SafeCaseNotification):
            raise SupportTransportRejected("authorized_notification_required")
        if isinstance(lease_seconds, bool) or not isinstance(lease_seconds, int) or lease_seconds < 1 or lease_seconds > 900:
            raise SupportTransportRejected("invalid_notification_lease")
        principal_digest = _digest(telegram_principal)
        fingerprint = self._delivery_fingerprint(notification, principal_digest)
        current_time = _now() if now is None else float(now)
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
                "SELECT * FROM notification_deliveries WHERE delivery_fingerprint=?", (fingerprint,)
            ).fetchone()
            ...
            if row and row["state"] == "CLAIMED" and float(row["claim_expires_at"] or 0) > current_time:
                connection.execute("COMMIT")
                return NotificationClaim(
                    delivery_id=row["delivery_id"],
                    idempotency_key=notification.idempotency_key,
            ...
            raw_claim = uuid.uuid4().hex
            claim_digest = sha256(raw_claim.encode("utf-8")).hexdigest()
            expires = current_time + lease_seconds
            if row:
                attempt_count = int(row["attempt_count"]) + 1
                delivery_id = row["delivery_id"]
                connection.execute(
                    "UPDATE notification_deliveries SET state='CLAIMED',claim_token_sha256=?,claim_expires_at=?,attempt_count=? WHERE delivery_id=?",
                    (claim_digest, expires, attempt_count, delivery_id),
                )
            else:
                attempt_count = 1
                delivery_id = "nd_" + uuid.uuid4().hex
                connection.execute(
    ...
    def mark_notification_delivered(
        ...
        *,
        ...
        if not all(isinstance(value, str) and value.strip() for value in (delivery_id, claim_token, transport_message_id)):
            raise SupportTransportRejected("invalid_delivery_receipt")
        claim_digest = sha256(claim_token.strip().encode("utf-8")).hexdigest()
        delivered_at = _now() if now is None else float(now)
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
                "SELECT * FROM notification_deliveries WHERE delivery_id=?", (delivery_id.strip(),)
            ).fetchone()
            if not row:
                raise SupportTransportRejected("delivery_not_found")
            if row["state"] == "DELIVERED":
                if row["transport_message_id"] != transport_message_id.strip():
                    raise SupportTransportRejected("delivery_receipt_conflict")
                connection.execute("COMMIT")
                return {"delivery_id": row["delivery_id"], "state": "DELIVERED", "idempotent": True}
            if row["claim_token_sha256"] != claim_digest:
                raise SupportTransportRejected("delivery_claim_invalid")
            connection.execute(
                "UPDATE notification_deliveries SET state='DELIVERED',transport_message_id=?,delivered_at=?,claim_token_sha256=NULL,claim_expires_at=NULL WHERE delivery_id=?",
                (transport_message_id.strip(), delivered_at, row["delivery_id"]),
            )
            connection.execute("COMMIT")
