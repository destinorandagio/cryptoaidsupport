...
RISK_TERMS = ("seed phrase", "private key", "password", "2fa", "recovery phrase")
CASE_TERMS = ("scam", "rug pull", "rugpull", "hacked", "hack", "stolen", "lost funds", "dead token", "dead dapp", "recovery")
EVIDENCE_TERMS = ("tx hash", "transaction hash", "wallet address", "contract address", "url", "screenshot")
...
def next_step(intent: Intent, language: str = "en") -> str:
    ...
    if intent.safety_risk:
        return ("Non inviare seed phrase, chiavi private, password o codici 2FA. "
                "Condividi solo prove pubbliche e non segrete." if it else
                "Never send seed phrases, private keys, passwords or 2FA codes. Share only public, non-secret evidence.")
