...
class TelegramRateGate:
    ...
    The lock covers prune + check + consume as one critical section so concurrent
    handlers cannot overbook the same chat/user envelope. This remains process-local;
    CHAT10 still owns distributed throttling, Telegram 429 backoff and durable receipts.
    """
    ...
    def allow(self, *, chat_id: object, user_id: object, now: float | None = None) -> bool:
        ...
        capacity in another scope. Prune/check/consume is serialized for thread safety.
        """
