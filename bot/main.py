...
TEXT = {
 "en": {
  ...
  "case": "🧭 CRYPTOAID CASE PRE-ASSESSMENT\nReply with: project/token/dApp name; what happened; approximate date; chain; public transaction hash/address/contract/URL if available.\n\n⚠️ Never send seed phrases, private keys, passwords or 2FA codes. No recovery outcome is guaranteed. Private Case status remains fail-closed until authenticated Case-owner authorization is available."
 },
 "it": {
  ...
  "case": "🧭 PRE-ASSESSMENT CASE CRYPTOAID\nRispondi indicando: progetto/token/dApp; cosa è successo; data approssimativa; chain; transaction hash/address/contract/URL pubblici se disponibili.\n\n⚠️ Mai seed phrase, chiavi private, password o 2FA. Nessun risultato di recovery è garantito. Lo stato privato del Case resta fail-closed finché non è disponibile l'autorizzazione autenticata del proprietario."
 }
}
...
async def rate_guard(update: Update) -> bool:
    ...
    if not allowed:
        # Deliberately do not emit a throttle message: replying when the outbound
        # envelope is exhausted can itself amplify a Telegram 429 condition.
...
async def new_members(update, context):
    if not update.message or not any(not member.is_bot for member in update.message.new_chat_members):
        return
    if await rate_guard(update):
        # One service update gets at most one welcome, even when several users join.
