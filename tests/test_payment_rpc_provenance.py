...
def new_intent(e, expected="450", key="rpc", case_id="case-rpc"):
    return e.create_payment_intent(
        case_id=case_id,
        entitlement_ref=f"case_active:{case_id}",
        payer="0xsender",
        asset="POL",
...
def test_server_rpc_provenance_settles_and_binds_certificate():
    ...
    result = adapter.settle_from_tx_hash(
...
def test_wrong_chain_sender_treasury_value_or_receipt_never_settles(overrides):
    e = engine()
    intent = new_intent(e)
    adapter = TrustedPolygonRPCAdapter(e, FakeRPC(intent, overrides=overrides))
    result = adapter.settle_from_tx_hash(
        intent_id=intent["intent_id"], tx_hash="0xabc", provider_ids=["rpc_a", "rpc_b"]
    )
...
def test_provider_economic_disagreement_is_manual_review():
    e = engine()
    intent = new_intent(e)
    rpc = FakeRPC(intent, overrides={"rpc_b": {"value": hex(451 * WEI)}})
...
def test_tx_receipt_block_or_hash_disagreement_fails_closed():
    e = engine()
    intent = new_intent(e)
    rpc = FakeRPC(intent, overrides={"rpc_b": {"block_hash": "0xdifferent"}})
    result = TrustedPolygonRPCAdapter(e, rpc).settle_from_tx_hash(
        intent_id=intent["intent_id"], tx_hash="0xabc", provider_ids=["rpc_a", "rpc_b"]
    )
    assert result["verdict"] == "MANUAL_REVIEW"
...
def test_duplicate_provider_identity_fails_closed_without_entitlement():
    ...
    rpc = FakeRPC(intent)
    result = TrustedPolygonRPCAdapter(e, rpc).settle_from_tx_hash(
        intent_id=intent["intent_id"], tx_hash="0xabc", provider_ids=["rpc_a", "rpc_a"]
    )
    assert result["verdict"] == "MANUAL_REVIEW"
    with e._connect() as c:
        assert c.execute("SELECT COUNT(*) FROM entitlement_ledger").fetchone()[0] == 0
...
def test_finality_pending_and_mixed_finality_are_distinct_fail_closed_states():
    ...
    assert pending_engine.get_intent(pending_intent["intent_id"])["state"] == "FINALITY_PENDING"
    ...
    assert mixed["verdict"] == "MANUAL_REVIEW"
    assert mixed_engine.get_intent(mixed_intent["intent_id"])["state"] == "MANUAL_REVIEW"


def test_rpc_adapter_preserves_activation50_first450_then500_model():
    e = engine()
    activation = e.create_activation_intent(
        principal_id="sic-rpc",
        payer="0xsender",
        request_id="req-activation-rpc",
        idempotency_key="idem-activation-rpc",
    )
    assert activation["expected_value"] == "50"
    activation_result = TrustedPolygonRPCAdapter(e, FakeRPC(activation, tx_hash="0xactivation")).settle_from_tx_hash(
        intent_id=activation["intent_id"], tx_hash="0xactivation", provider_ids=["rpc_a", "rpc_b"]
    )
    assert activation_result["verdict"] == "SETTLED"
    assert e.get_activation_credit("sic-rpc")["state"] == "AVAILABLE"

    first = e.create_case_payment_intent(
        principal_id="sic-rpc",
        case_id="case-first",
        payer="0xsender",
        request_id="req-first-rpc",
        idempotency_key="idem-first-rpc",
    )
    assert first["expected_value"] == "450"
    first_result = TrustedPolygonRPCAdapter(e, FakeRPC(first, tx_hash="0xfirst")).settle_from_tx_hash(
        intent_id=first["intent_id"], tx_hash="0xfirst", provider_ids=["rpc_a", "rpc_b"]
    )
    assert first_result["verdict"] == "SETTLED"
    assert e.get_activation_credit("sic-rpc")["state"] == "CONSUMED"

    later = e.create_case_payment_intent(
        principal_id="sic-rpc",
        case_id="case-later",
        payer="0xsender",
        request_id="req-later-rpc",
        idempotency_key="idem-later-rpc",
    )
...
def test_same_rpc_tx_cannot_settle_two_intents():
    e = engine()
    first = new_intent(e, key="first", case_id="case-first")
    assert TrustedPolygonRPCAdapter(e, FakeRPC(first)).settle_from_tx_hash(
        intent_id=first["intent_id"], tx_hash="0xabc", provider_ids=["rpc_a", "rpc_b"]
    )["verdict"] == "SETTLED"

    second = new_intent(e, key="second", case_id="case-second")
    result = TrustedPolygonRPCAdapter(e, FakeRPC(second)).settle_from_tx_hash(
        intent_id=second["intent_id"], tx_hash="0xabc", provider_ids=["rpc_a", "rpc_b"]
    )
    assert result["verdict"] == "MANUAL_REVIEW"
    assert result["entitlement_granted"] is False
