...
def fixture(tmp_path: Path):
    ...
    owner = core.register_user("SIC-TRANSPORT-OWNER", {}, "reg-owner", "req-owner")
    other = core.register_user("SIC-TRANSPORT-OTHER", {}, "reg-other", "req-other")
    ...
    case = core.open_case(
        ...
        "USER", "req-case-transport", "idem-case-transport",
    )
...
def test_authorized_ticket_is_durable_idempotent_and_principal_is_hashed(tmp_path: Path):
    ...
    first = durable.create_ticket(
        telegram_principal=principal,
        support_session_id=token,
        case_id=case["case_id"],
        summary="Please review my Case status and next action.",
        category="CASE_STATUS",
    )
    replay = durable.create_ticket(
        telegram_principal=principal,
        support_session_id=token,
        case_id=case["case_id"],
        summary="Please review my Case status and next action.",
    ...
    with sqlite3.connect(db) as connection:
        row = connection.execute("SELECT case_id,category,escalate,summary FROM support_tickets").fetchone()
        assert row == (case["case_id"], "CASE_STATUS", 0, "Please review my Case status and next action.")
...
def test_cross_user_revoked_and_secret_ticket_fail_before_persistence(tmp_path: Path):
    ...
    with pytest.raises(TelegramDurableSupportRejected):
        durable.create_ticket(
    ...
    with pytest.raises(TelegramDurableSupportRejected):
        durable.create_ticket(
            ...
            case_id=case["case_id"],
            summary="my seed phrase is alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu",
        )
...
def test_notification_claim_is_single_winner_then_delivery_is_deduped(tmp_path: Path):
    _, private, durable, db, owner, _, session, _, case = fixture(tmp_path)
    principal = "telegram:notify-owner"
    token = bind(private, session, owner["sic_id"], principal)

    def claim():
        return durable.claim_notification(
            telegram_principal=principal,
            support_session_id=token,
            case_id=case["case_id"],
    ...
    winners = [claim for claim in claims if claim.should_send]
    assert len(winners) == 1
    winner = winners[0]
    assert winner.claim_token
    receipt = durable.mark_notification_delivered(
        delivery_id=winner.delivery_id,
        claim_token=winner.claim_token,
        transport_message_id="telegram-message-42",
        now=1001.0,
    )
    assert receipt == {"delivery_id": winner.delivery_id, "state": "DELIVERED", "idempotent": False}
    after = claim()
    assert after.should_send is False and after.state == "DELIVERED"
...
def test_expired_claim_can_be_reclaimed_but_old_claim_cannot_ack(tmp_path: Path):
    _, private, durable, _, owner, _, session, _, case = fixture(tmp_path)
    principal = "telegram:lease-owner"
    token = bind(private, session, owner["sic_id"], principal)
    first = durable.claim_notification(
        telegram_principal=principal,
        support_session_id=token,
        case_id=case["case_id"],
    ...
    second = durable.claim_notification(
        telegram_principal=principal,
        support_session_id=token,
        case_id=case["case_id"],
        event_type="ACTION_REQUIRED",
    ...
    assert first.should_send and second.should_send
    assert second.attempt_count == 2 and second.claim_token != first.claim_token
    with pytest.raises(TelegramDurableSupportRejected):
        durable.mark_notification_delivered(
            delivery_id=first.delivery_id,
            claim_token=first.claim_token,
            transport_message_id="stale",
        )
    durable.mark_notification_delivered(
        delivery_id=second.delivery_id,
        claim_token=second.claim_token,
        transport_message_id="fresh",
    )


def test_transport_db_rejects_public_html_and_integrity_is_ok(tmp_path: Path):
    with pytest.raises(SupportTransportRejected):
        SupportTransportStore(tmp_path / "public_html" / "support.sqlite")
    store = SupportTransportStore(tmp_path / "private" / "support.sqlite")
