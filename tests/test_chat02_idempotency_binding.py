...
def test_exact_generic_replay_is_idempotent_without_duplicate_event():
    ...
    binding = e.get_idempotency_binding("idem-shared")
    assert binding and binding["operation"] == "GENERIC"
    with e._connect() as c:
...
def test_activation_same_key_changed_principal_or_payer_fails_closed():
    e = engine()
    first = e.create_activation_intent(principal_id="sic-a", payer="0xpayer-a", request_id="req-a", idempotency_key="idem-activation")
    for principal, payer in (("sic-b", "0xpayer-a"), ("sic-a", "0xpayer-b")):
        with pytest.raises(EvidencePaymentError) as exc:
            e.create_activation_intent(principal_id=principal, payer=payer, request_id="req-a", idempotency_key="idem-activation")
        assert exc.value.code == "IDEMPOTENCY_CONFLICT"
...
def test_case_same_key_changed_principal_case_payer_or_request_fails_closed_without_credit_leakage():
    e = engine()
    ...
    exact = e.create_case_payment_intent(principal_id="sic-a", case_id="case-a", payer="0xpayer-a", request_id="req-a", idempotency_key="idem-case")
    assert exact["intent_id"] == first["intent_id"]
    ...
    conflicts = [
        ...
        dict(principal_id="sic-a", case_id="case-a", payer="0xpayer-a", request_id="req-b"),
    ]
...
def test_same_key_cannot_cross_operation_boundary():
    e = engine()
    create_generic(e, key="idem-cross")
    with pytest.raises(EvidencePaymentError) as exc:
        e.create_activation_intent(principal_id="sic-a", payer="0xpayer-a", request_id="req-a", idempotency_key="idem-cross")
    assert exc.value.code == "IDEMPOTENCY_CONFLICT"
    assert_integrity(e)
...
def test_conflicting_concurrent_same_key_has_one_winner_and_one_fail_closed():
    ...
    assert sum(r[0] == "error" and r[1] == "IDEMPOTENCY_CONFLICT" for r in results) == 1
    with e._connect() as c:
        assert c.execute("SELECT COUNT(*) FROM payment_intents WHERE idempotency_key='idem-race'").fetchone()[0] == 1
        assert c.execute("SELECT COUNT(*) FROM payment_idempotency_bindings WHERE idempotency_key='idem-race'").fetchone()[0] == 1
    assert_integrity(e)
